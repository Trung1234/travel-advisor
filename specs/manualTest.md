...
Confirm that the Streamlit UI, FastAPI backend, model provider, and TripAdvisor enrichment work together for a basic voice-first travel flow.
...
- `.env` is configured with the provider and TripAdvisor settings.
...
- TripAdvisor credentials are available if you want to verify hotel/destination enrichment.
...
- If TripAdvisor data is available, confirm it influences the answer.
...
- If TripAdvisor data is available, confirm it is used.
...
| TripAdvisor-enhanced destination response works | [ ] |  |
| TripAdvisor-enhanced hotel response works | [ ] |  |
...
- Whether TripAdvisor enrichment was enabled
- What worked
- What failed
...
The app should complete a full travel Q&A flow without crashing, and it should respond gracefully if the provider or TripAdvisor data is unavailable.
